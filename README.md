...
*Matrix last verified against code: 2026-08-29.*
...
| Server (`server/`) | Functional — REST (`/health` `/worlds` `/world` `/agent` `/observe` `/puzzle` `/players` `/history` `/chronicle` `/wayback` `/image` `/speak` `/puzzle/attempt` `/act` `/agent/voice` `/position` `/client-error`), player guide at `/guide`, WebSocket multiplayer at `/ws`, co-op puzzle sessions, bundled browser UI at `/app`, security headers + CSP, body/frame size caps. **Canonical-world boundary:** `NESTED_WORLDS_CANONICAL_SEED` governs every HTTP, SSE, WebSocket, position, and heartbeat path; missing seed selects it, a mismatch is rejected before lookup/birth, and stale alternate-world positions cannot redirect a player. **Node identity is server-derived**: `/speak`, `/image`, `/agent/voice`, and `/wayback` resolve the named node against that world (404 for forged names). WebSockets retain strict RFC 6455 framing and non-blocking per-player writer queues; agents remain addressable through persisted memory and node-scoped history |
...
| CLI (`main.py`) | Functional — `world`, `agent`, `puzzles`, `play`, `serve`, `speak`, `history`, and read-only `work-report` (all three delayed-work queues); `--seed` remains accepted for operator curation, deterministic evaluation, and isolated local development. To act in the hosted world's history, operators use its configured canonical seed. Player-facing browser clients cannot choose or create worlds |
...
| Frontend (`frontend/`) | Functional — React + PixiJS + Vite and vanilla-D3 clients wired to the same server-owned world; neither exposes seed/world creation. Node conversation, puzzles, speak-to-presences, passage affordances, multiplayer presence, and chronicle all converge on the shared history. **Wayback (ADR-011) runs in both clients:** scrub or animate any node from birth through its recorded event steps, rendered by today's deterministic local art and opt-in sound — historical state through present senses, explicitly not period playback. First witness is derived; the archive creates no trace and never classifies one as human or agent. **Per-node generative art** (`static/nodeart.js`, with the state-driven scene in `static/sensory.js`) and **per-scale musical forms** (`static/score.js`, eleven sampled forms directed by each node's served senses) are shared by both clients: scale, properties, history, causal pressure, and condition shape what each place looks and sounds like. Sound stays opt-in for browser activation; fal.ai imagery is an optional enhancement wash; WebGL failure degrades gracefully |
...
| Tests | 903 Python tests across generator and stored-world continuity, agents, puzzle quality/ecology, effects and staged causality, persistence, chronicled deltas, state-at-T/Wayback, the wrap passage, consciousness, heartbeat, HTTP/WebSocket conformance, canonical-world boundaries, node resolution, beta guards, deployment contracts, frontend↔endpoint contracts, and observability — plus 95 Vitest tests for canonical entry/resume/affordance/chronicle/wrap/Wayback/display-name behavior, WebSocket dispatch, and deterministic node art and sound. CI also builds the production frontend, rejects a stale committed bundle, smoke-tests the installed wheel, and runs Playwright against the real server under the production CSP. |
...
scale-native actions in a single Act tab. The printed link includes a disposable local invite.
No model key is needed for choosing/combining actions, images or score.
The player guide lists the current action vocabulary. Use `--resume /path/to/preview.db`
to retain a saved local review world rather than starting over.
`node scripts/render_score_audition.mjs http://127.0.0.1:8201 /tmp/enfolded-audition`
creates eleven local listening excerpts with concealed scale labels. See the
