...
| Tests | 504 Python tests across generator (incl. canonical prefix-stability), agents (incl. memory-across-rebuilds and puzzle-rule parity), puzzles (quality invariants — no-leak, solvable, node-unique, per-node difficulty spread, transform integrity, never-in-properties), effects + causal wiring, persistence (incl. invite keys + cross-device position + property overlay + restart-proof co-op), causality, interface, consciousness (incl. transcripts, fallback voices, bible cache-effectiveness), heartbeat, HTTP/WebSocket server, node resolution + agent addressability, beta guards, Fly deploy config, frontend↔endpoint contract, and observability — plus 29 Vitest JS tests (entry + affordance cross-client parity, WS dispatch) run in CI |
