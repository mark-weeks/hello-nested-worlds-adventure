...
| Tests | 417 tests across generator, agents, puzzles (quality invariants — no-leak, solvable, node-unique, per-node difficulty spread, transform integrity), persistence (incl. invite keys), causality, interface, consciousness, HTTP/WebSocket server, beta guards (incl. per-user keys + per-user cost caps + world-size bound), Fly deploy config, frontend↔endpoint contract (incl. difficulty rating), and observability |
